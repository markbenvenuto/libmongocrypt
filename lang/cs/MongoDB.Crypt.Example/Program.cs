...
namespace drivertest
{
    ...
    {
        readonly IKmsKeyId _kmsKey;
        MongoClient _clientCryptD;
        IMongoCollection<BsonDocument> _keyVault;
        Uri _kmsEndpoint;

        public MongoCryptDController(MongoUrl urlCryptD, IMongoCollection<BsonDocument> keyVault, Uri kmsEndpoint, IKmsKeyId kmsKey)
        {
        ...
        public BsonDocument EncryptCommand(IMongoCollection<BsonDocument> coll, BsonDocument cmd)
        {
            CryptOptions options = new CryptOptions();
            {
                ...
                using (var context = foo.StartEncryptionContext(coll.CollectionNamespace.FullName, _kmsKey))
                {
        ...
        public BsonDocument DecryptCommand(IMongoDatabase db, BsonDocument doc)
        {
            CryptOptions options = new CryptOptions();
            {
                ...
                using (var foo = CryptClientFactory.Create(options))
                using (var context = foo.StartDecryptionContext(BsonUtil.ToBytes(doc), _kmsKey))
                {
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            AwsKeyId aws = new AwsKeyId() { CustomerMasterKey = "cmk", Region = "us-east-1" };
            ...
            var controller = new MongoCryptDController(cryptDUrl, collKeyVault, kmsURL, aws);
